...
# %%
print(
    f"Duplicates in train set: {train.duplicated().sum()}, ({np.round(100 * train.duplicated().sum() / len(train), 1)}%)"
)
print("")
print(
    f"Duplicates in test set: {test.duplicated().sum()}, ({np.round(100 * test.duplicated().sum() / len(test), 1)}%)"
)
...
for df in [train, test]:
    max_age = df["Age"].max()
    df["Age_group"] = pd.cut(
        df["Age"], bins=[0, 12, 17, 25, 30, 50, max_age], labels=age_groups_labels
    )

# Plot distribution of new features
g = sns.countplot(data=train, x="Age_group", hue="Transported", order=age_groups_labels)
...
train["Cabin_region2"] = (
    (train["Cabin_number"] >= 300) & (train["Cabin_number"] < 600)
).astype(int)
train["Cabin_region3"] = (
    (train["Cabin_number"] >= 600) & (train["Cabin_number"] < 900)
).astype(int)
train["Cabin_region4"] = (
    (train["Cabin_number"] >= 900) & (train["Cabin_number"] < 1200)
).astype(int)
train["Cabin_region5"] = (
    (train["Cabin_number"] >= 1200) & (train["Cabin_number"] < 1500)
).astype(int)
train["Cabin_region6"] = (
    (train["Cabin_number"] >= 1500) & (train["Cabin_number"] < 1800)
).astype(int)
...
test["Cabin_region2"] = (
    (test["Cabin_number"] >= 300) & (test["Cabin_number"] < 600)
).astype(int)
test["Cabin_region3"] = (
    (test["Cabin_number"] >= 600) & (test["Cabin_number"] < 900)
).astype(int)
test["Cabin_region4"] = (
    (test["Cabin_number"] >= 900) & (test["Cabin_number"] < 1200)
).astype(int)
test["Cabin_region5"] = (
    (test["Cabin_number"] >= 1200) & (test["Cabin_number"] < 1500)
).astype(int)
test["Cabin_region6"] = (
    (test["Cabin_number"] >= 1500) & (test["Cabin_number"] < 1800)
).astype(int)
...
train["Family_size"] = train["Surname"].map(
    lambda x: pd.concat([train["Surname"], test["Surname"]]).value_counts()[x]
)
test["Family_size"] = test["Surname"].map(
    lambda x: pd.concat([train["Surname"], test["Surname"]]).value_counts()[x]
)
...
CDHP_gb = (
    data.groupby(["Cabin_deck", "HomePlanet"])["HomePlanet"].size().unstack().fillna(0)
)
...
data.loc[
    (data["HomePlanet"].isna()) & (data["Cabin_deck"].isin(["A", "B", "C", "T"])),
    "HomePlanet",
] = "Europa"

# Deck G came from Earth
data.loc[(data["HomePlanet"].isna()) & (data["Cabin_deck"] == "G"), "HomePlanet"] = (
    "Earth"
)
...
SHP_gb = (
    data.groupby(["Surname", "HomePlanet"])["HomePlanet"].size().unstack().fillna(0)
)
...
SHP_index = data[data["HomePlanet"].isna()][
    (data[data["HomePlanet"].isna()]["Surname"]).isin(SHP_gb.index)
].index

# Fill corresponding missing values
data.loc[SHP_index, "HomePlanet"] = data.iloc[SHP_index, :]["Surname"].map(
    lambda x: SHP_gb.idxmax(axis=1)[x]
)
...
HPD_gb = (
    data.groupby(["HomePlanet", "Destination"])["Destination"]
    .size()
    .unstack()
    .fillna(0)
)
...
data.loc[(data["HomePlanet"].isna()) & ~(data["Cabin_deck"] == "D"), "HomePlanet"] = (
    "Earth"
)
data.loc[(data["HomePlanet"].isna()) & (data["Cabin_deck"] == "D"), "HomePlanet"] = (
    "Mars"
...
GCD_gb = (
    data[data["Group_size"] > 1]
    .groupby(["Group", "Cabin_deck"])["Cabin_deck"]
    .size()
    .unstack()
    .fillna(0)
)
GCN_gb = (
    data[data["Group_size"] > 1]
    .groupby(["Group", "Cabin_number"])["Cabin_number"]
    .size()
    .unstack()
    .fillna(0)
)
GCS_gb = (
    data[data["Group_size"] > 1]
    .groupby(["Group", "Cabin_side"])["Cabin_side"]
    .size()
    .unstack()
    .fillna(0)
)
...
GCS_index = data[data["Cabin_side"].isna()][
    (data[data["Cabin_side"].isna()]["Group"]).isin(GCS_gb.index)
].index

# Fill corresponding missing values
data.loc[GCS_index, "Cabin_side"] = data.iloc[GCS_index, :]["Group"].map(
...
SCS_gb = (
    data[data["Group_size"] > 1]
    .groupby(["Surname", "Cabin_side"])["Cabin_side"]
    .size()
    .unstack()
    .fillna(0)
)
...
# Print proportion
print(
    "Percentage of families all on the same cabin side:",
...
SCS_index = data[data["Cabin_side"].isna()][
    (data[data["Cabin_side"].isna()]["Surname"]).isin(SCS_gb.index)
].index

# Fill corresponding missing values
data.loc[SCS_index, "Cabin_side"] = data.iloc[SCS_index, :]["Surname"].map(
...
GCD_index = data[data["Cabin_deck"].isna()][
    (data[data["Cabin_deck"].isna()]["Group"]).isin(GCD_gb.index)
].index

# Fill corresponding missing values
data.loc[GCD_index, "Cabin_deck"] = data.iloc[GCD_index, :]["Group"].map(
...
# %%
# Joint distribution
data.groupby(["HomePlanet", "Destination", "Solo", "Cabin_deck"])[
    "Cabin_deck"
...
sns.scatterplot(
    x=data["Cabin_number"],
    y=data["Group"],
    c=LabelEncoder().fit_transform(
        data.loc[~data["Cabin_number"].isna(), "Cabin_deck"]
    ),
    cmap="tab10",
)
...
for deck in ["A", "B", "C", "D", "E", "F", "G"]:
    # Features and labels
    X_CN = data.loc[
        ~(data["Cabin_number"].isna()) & (data["Cabin_deck"] == deck), "Group"
    ]
    y_CN = data.loc[
        ~(data["Cabin_number"].isna()) & (data["Cabin_deck"] == deck), "Cabin_number"
    ]
    X_test_CN = data.loc[
        (data["Cabin_number"].isna()) & (data["Cabin_deck"] == deck), "Group"
    ]
...
data["Cabin_region2"] = (
    (data["Cabin_number"] >= 300) & (data["Cabin_number"] < 600)
).astype(int)
data["Cabin_region3"] = (
    (data["Cabin_number"] >= 600) & (data["Cabin_number"] < 900)
).astype(int)
data["Cabin_region4"] = (
    (data["Cabin_number"] >= 900) & (data["Cabin_number"] < 1200)
).astype(int)
data["Cabin_region5"] = (
    (data["Cabin_number"] >= 1200) & (data["Cabin_number"] < 1500)
).astype(int)
data["Cabin_region6"] = (
    (data["Cabin_number"] >= 1500) & (data["Cabin_number"] < 1800)
).astype(int)
...
# %%
# Joint distribution
data.groupby(["HomePlanet", "No_spending", "Solo", "Cabin_deck"])[
    "Age"
...
# %%
# Joint distribution
data.groupby(["HomePlanet", "Solo", "Age_group"])[
    "Expenditure"
...
for col in exp_feats:
    ...
    data.loc[data[col].isna(), col] = data.groupby(["HomePlanet", "Solo", "Age_group"])[
        col
